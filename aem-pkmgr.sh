...
function help
{
    ...
    echo " uninstall       - uninstalls a package"
    ...
    echo "-pk | --package  - Sets the package to install/uninstall, delete or upload"
...
uninstall_package ()
{
    if [ "$PACKAGE" == "0" ]; then
        echo "Missing package!"
        echo "usage: aem-pkmgr uninstall -pk [package-path]"
        exit 1
    fi
    echo "UNINSTALLING PACKAGE $PACKAGE..."
    encoded_package=${PACKAGE// /%20}
    curl -u $USER:$PASSWORD -X POST --fail "$HOST/crx/packmgr/service/.json$encoded_package?cmd=uninstall"
    echo ""
}
...
delete_package ()
{
    if [ "$PACKAGE" == "0" ]; then
        echo "Missing package!"
        echo "usage: aem-pkmgr delete -pk [package-path]"
        exit 1
    fi
    echo "DELETING PACKAGE $PACKAGE..."
    encoded_package=${PACKAGE// /%20}
    curl -u $USER:$PASSWORD -X POST --fail "$HOST/crx/packmgr/service/.json$encoded_package?cmd=delete"
    echo ""
}
